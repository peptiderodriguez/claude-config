...
if printf '%s\n' "$cmd" | grep -qE '(^|[[:space:];&|])scancel[[:space:]]+-u(\b|[[:space:]=])'; then
  jq -nc --arg reason 'Blocked: `scancel -u` is destructive across cubes (past incident wiped a 7-hour libgen). Cancel by explicit job-id list instead: `scancel <jobid1> <jobid2> ...`. To override this guard intentionally, edit ~/.claude/hooks/scancel_guard.sh.' \
