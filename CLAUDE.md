...
- **Claims defended by stacks, not single pieces.** Trust is layered: regression test + end-to-end smoke + per-row provenance + CI-gated thresholds + PROVISIONAL labels on under-powered claims. No single piece is load-bearing. **Trigger:** *"a green test that coexists with a broken end-to-end DAG is worse than no test"* (binder-design P-04 scar). Every test must answer *"does this catch failures that block our actual goal — arbitrary input → orderable output?"* If the test is green but the DAG is broken downstream and nothing caught it, the test is decoration. **Trigger (independent-oracle):** a gate must validate against a source *independent* of the one the artifact was generated from. If a check's input and its oracle share a generator — e.g. a citations CSV audited against the same cache it was *populated from* — it passes *because* it cannot fail; that's decoration, not a layer. The hole stays invisible until a wrong-but-internally-consistent value ships green (the binder-design citation gate shipped 3 known-fabricated PMIDs as `live` this way). Add a layer that can actually disagree: an external source, a held-out truth, or a hand-adversarial row.
...
- **Sub-agent writes: possible, but gated OFF by default.** A subagent CAN Write/Edit once the cwd's `.claude/settings.local.json` grants a `Write(<repo>/**)` / `Edit(<repo>/**)` allow-rule — subagents inherit the parent's permission scope (the old "auto-denied, can't write" belief was wrong). BUT uncontrolled parallel agent writes caused a concurrent-merge never-event (two workflows wrote the same file), so `hooks/agent_write_guard.sh` **denies subagent/workflow writes by default** — it detects them via the call's top-level `.agent_id` (main-thread writes pass) and logs every decision to `<cwd>/.claude/agent_write_guard.log`. **Default working pattern stays:** spawn agents for read-only research / analysis; have each RETURN exact edits (diffs, residue lists, YAML blocks) as text; apply the Writes yourself as the single write-stream. **To intentionally arm one isolated writer:** `touch <cwd>/.claude/.allow_agent_writes` AND run it `isolation: "worktree"` (never two writers on one file). *"A brief cannot prevent a write; only a gate can — write only when you mean to write."*
