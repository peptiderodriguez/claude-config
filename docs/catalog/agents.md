...
!!! note "How they register"
    Claude Code discovers `~/.claude/agents/*.md` at session start and registers each as a `subagent_type` from its `name:` frontmatter field (reload with `/agents` after install). If a particular runtime exposes only a curated agent set and `Agent(subagent_type="dfg-reviewer", …)` returns "agent type not found", fall back to `subagent_type="general-purpose"` with the agent's prompt body embedded inline.
