...
**Why:** Verbatim from `clinical-omics/.claude/commands/analyze.md`: *"We've burned this lab once already with a fabricated 27-protein 'Thienel panel' that had 7+ proteins not in the paper. If the user shares a PDF, extract its text and grep before claiming any gene is 'from the paper'."*
...
- This applies across projects, not just `clinical-omics` — anywhere literature-grounded panels are needed (binder-design hit interpretation, grant-repo biomarker selection, grant-prep panels).
